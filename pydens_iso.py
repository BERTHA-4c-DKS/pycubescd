from __future__ import absolute_import
import matplotlib.pyplot as plt
import numpy as np 
import argparse
from scipy.interpolate import RegularGridInterpolator
from scipy.interpolate import interp1d
from scipy.optimize import newton
from scipy.optimize import fmin 

import os
import sys
import os.path
from io import open

sys.path.append(u"./modules")
import load_cube

parser = argparse.ArgumentParser()
parser.add_argument(u"-f1",   u"--filefrag1", help=u"cube format file of molecular fragment 1", \
        type=unicode, required=True)
parser.add_argument(u"-f2",   u"--filefrag2", help=u"cube format file of molecular fragment 2", \
        type=unicode, required=True)
parser.add_argument(u"-iseed",   u"--initialseed", help=u"initial seed (real numer). An estimated value of the isodensity value", \
        type=unicode, required=True)
parser.add_argument(u"-o_iso",u"--outputiso",  help=u"Output file name", \
        type=unicode, default=u"stdout")
parser.add_argument(u"-axis", help=u"Specify the axis on which evaluating the isodensity value [x,y,z]",\
        type=unicode, default=u"z")
parser.add_argument(u"--verbose", help=u"increase output verbosity", \
        default=False, action=u"store_true")

if len(sys.argv) == 1:
    parser.print_help()
    exit(1)

args = parser.parse_args()

if args.verbose:
   print u"verbosity turned on"  

if (args.axis != u'z' and args.axis != u'y' and args.axis != u'x'):
   args.axis = u'z' 
   print u'Problem with the axis definition, we set it to the default z value.'

print (u'Reading...' + args.filefrag1)
mycube = load_cube.cube()

if not (os.path.isfile(args.filefrag1)):
    print u"File ", args.filefrag1, u" does not exist "
    exit(1)

mycube.readfile(args.filefrag1)
x, y, z = np.array(mycube.get_grid_xyz())

data = mycube.get_data()

print (u'Interpolating...' + args.filefrag1)
my_interpolating_function1 = RegularGridInterpolator((x,y,z),data, method =u'linear')

print (u'Reading...' + args.filefrag2)
mycube = load_cube.cube()

if not (os.path.isfile(args.filefrag2)):
    print u"File ", args.filefrag2, u" does not exist "
    exit(1)



mycube.readfile(args.filefrag2)
x2,y2,z2 = np.array(mycube.get_grid_xyz())
data = mycube.get_data()

print (u'Interpolating...' + args.filefrag2)
my_interpolating_function2 = RegularGridInterpolator((x2,y2,z2),data, method =u'linear')




#print('min z...', np.min(z))
#print('max z...', np.max(z))
#print('min z2...', np.min(z2))
#print('max z2...', np.max(z2))
#print('max(min(z,z2)', np.max(np.array([np.min(z),np.min(z2)])))
#print('min(max(z,z2)', np.min(np.array([np.max(z),np.max(z2)])))

print u'IMPORTANT...it is required that the intersection of two cubes is/= O '
print u'Explicit check is not done by the program '
# TO DO

# This is for fixing the range of sampling point (info arises from the cube grid)
nump = 40000   # Fix number of sampling points

if (args.axis == u'z'):

    
     xmin = np.max(np.array([np.min(z),np.min(z2)])) 
     xmax = np.min(np.array([np.max(z),np.max(z2)]))

      

     xpt1 = np.zeros(nump)
     xpt2 = np.zeros(nump)
     xpt3 = np.linspace(xmin,xmax,nump)
     xpt  = xpt3

elif (args.axis == u'y'):

     xmin = np.max(np.array([np.min(y),np.min(y2)]))
     xmax = np.min(np.array([np.max(y),np.max(y2)]))


     xpt1 = np.zeros(nump)
     xpt2 = np.linspace(xmin,xmax,nump) 
     xpt3 = np.zeros(nump)
     xpt  = xpt2

elif (args.axis == u'x'):

     xmin = np.max(np.array([np.min(x),np.min(x2)]))
     xmax = np.min(np.array([np.max(x),np.max(x2)]))

             
     xpt1 = np.linspace(xmin,xmax,nump)
     xpt2 = np.zeros(nump)
     xpt3 = np.zeros(nump)
     xpt  = xpt1

print (u'info... of sampling (nump, args.axis)', nump, args.axis)

pts = np.transpose([xpt1,xpt2,xpt3])

y1 = my_interpolating_function1(pts)
y2 = my_interpolating_function2(pts)


print u'Interpol 1D..linear.'
ydiff = (y2 - y1)**4
fdiff = interp1d(xpt, ydiff, kind = u'linear')
try:
    isodensity_point = fmin(fdiff,args.initialseed)  #  find a root Note that your stating point should be close to the final result
    print (u'isodensity_point =',isodensity_point)

except ValueError :
    print u'Oops problem in newton algorithm'


if (args.axis == u'z'):
      isodensity_value = my_interpolating_function2([0.0,0.0,float(isodensity_point)])   
      print (u'isodensity_value=',isodensity_value)

if (args.axis == u'y'):
      isodensity_value = my_interpolating_function1([0.0,float(isodensity_point),0.0])
      print (u'isodensity_value=',isodensity_value)

if (args.axis == u'x'):
      isodensity_value = my_interpolating_function1([float(isodensity_point),0.0,0.0])
      print (u'isodensity_value=',isodensity_value)



#print(args.outputiso)

if os.path.exists(args.outputiso):
    print u"File ", args.outputiso, u" exist, removing it "
    os.remove(args.outputiso)

f = open(args.outputiso, u'w')
f.write((u'Isodensity point at %f a.u. along axis %s Isodensity value of %f e/(a.u.)^3 \n') % (isodensity_point, args.axis, isodensity_value) )
f.close()

#plt.plot(xpt,ydiff)
plt.plot(xpt,y1)
plt.plot(xpt,y2)

text = u'isodensity point is' +unicode(isodensity_point)
plt.annotate(text, xy=(isodensity_point,0.005), xytext=(isodensity_point,0.005), \
        arrowprops=dict(facecolor=u'black', shrink=0.05))

text1 =u'isodensity value is' +unicode(isodensity_value)
plt.annotate(text1, xy=(isodensity_point,0.005), xytext=(isodensity_point,0.05))

plt.ylim((-0.1,1.0))
plt.xlabel(u'r (a.u)')


outfilename = args.outputiso + u".eps"

if os.path.exists(outfilename):
    print u"File ", outfilename, u" exist, removing it "
    os.remove(outfilename)

print u"Dumping file ", outfilename
plt.savefig(outfilename)
#plt.show()

#zz = mycube.cdz('cdz.out')
#zz = mycube.cdy('cdy.out')
#zz = mycube.cdx('cdx.out')
#mycube.toXYZ()
#b.read_cube('drho1.cube')
#c = a 
#c.print_cube('test.cub')
#x = np.transpose(np.array(zz))[0]
#y = np.transpose(np.array(zz))[1]
#plt.plot(x,y)
#plt.show()

#xpt1 = np.zeros(10000)
#xpt2 = np.zeros(10000)
#xpt1 = np.full(10000,0.142727)
#xpt2 = np.full(10000,0.142727)
#xpt3 = np.linspace(-10,10,10000)
#pts = np.transpose([xpt1,xpt2,xpt3])
#my_interpolating_function(pts)
#for i in pts.tolist():
#         print(('%e %e %e %e') % (i[0], i[1], i[2], my_interpolating_function(np.array(i))))
#print(np.transpose(pts)[2],my_interpolating_function(pts))
#plt.plot(np.transpose(pts)[2],my_interpolating_function(pts))
#plt.show()
